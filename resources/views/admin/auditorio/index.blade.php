...
<div class="col-12">
  <div class="card-box">
      @if(!isset($auditorio->id))
        <p id="req-cad">
          As informações do auditório ainda não foram cadastradas,
          <a id="cad" class="text-success" href="#"> Cadastre agora.</a>
        </p>
        <div id="form-cad" class="col-sm-12 col-xs-12" style="display:none">
          {{ Form::open(['route' => 'auditorio.store','files' => true]) }}
      @else
          <div id="form-cad" class="col-sm-12 col-xs-12">
          {{ Form::model($auditorio, ['route' => ['auditorio.update', $auditorio->id], 'method' => 'PUT','files' => true]) }}
      @endif

      <div class="row">
        <div class="form-group col-md-8">
            {{ Form::label('titulo', 'Titulo') }}
            {{ Form::text('titulo', null, array('class' => 'form-control', 'autofocus','maxlength' => '255', 'required')) }}
        </div>
        <div class="form-group col-md-4">
            {{ Form::label('imagem', 'Imagem') }}
            <input type="file" name="imagem" class="filestyle" data-placeholder="{{$auditorio->imagem}}" data-btnClass="btn-light">
        </div>
      </div>
      <div class="row">
        <div class="form-group col-md-12">
            {{ Form::label('texto', 'Texto') }}
            {{ Form::textarea('texto', null, array('class' => 'form-control', 'autofocus','maxlength' => '3500', 'required')) }}
        </div>
      </div>
    </div>
      ...
      <div class="row" style="margin-top: 20px">
        <div class="col-12">
          <div class="text-center">
            ...
            <a href="{{ route('auditorio.index') }}" class="btn btn-danger"><i class="dripicons-cross"></i> Cancelar</a>
          </div>
        </div>
      </div>
  </div>
